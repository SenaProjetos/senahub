...
Capture:
- whether `graphify-out/` already exists (that is the state dir; `.graphify/` is not, and there is no `graphify migrate-state`)
...
- read `GRAPH_REPORT.md` first, then `graphify god-nodes`
- one focused `explain`, `path`, or `affected` step instead of a blind rebuild — reach for `query --budget N` only when the symbol is unknown, since it is BFS and saturates on large graphs
