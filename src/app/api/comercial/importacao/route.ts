...
import { autoMapearCrm } from "@/lib/import/mapeamento-crm";

/** Upload + parse da planilha (preview) do Comercial (F4.5). Espelha `/api/financeiro/importacao`. */
export async function POST(req: Request) {
  ...
  const caminho = `comercial/importacao/${randomUUID()}__${slug(nomeArquivo)}`;
  ...
  return NextResponse.json({
    ...
    autoMap: autoMapearCrm(planilha.headers),
  });
}
