...
REM ============================================================
REM  Cabecalho: branch + ahead/behind + arvore limpa/suja
REM ============================================================
:git_header
set "GITBRANCH=?"
set "GITAHEAD=-"
...
echo   Branch: %GITBRANCH%   ^|   ahead: %GITAHEAD%  behind: %GITBEHIND%   ^|   arvore: %ARV%
exit /b
...
echo   10. Diagnostico (doctor, processos, limpar) ...
echo(
echo   11. Ajuda / Sobre
echo    0. Sair
echo(
...
echo(
echo   Log de auditoria deste menu: logs\dev-audit.log
...
if "%subop%"=="3" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -File "%PS1%" -Acao TestesArquivo -Sub "!padrao!"
  pause
  goto :menu_test
)
