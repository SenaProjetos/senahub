...
**✅ G2 entregue 2026-09-12 (Opus 5) — `financeiro:folha_pj_corrigir`, segregação de funções (D37):**
- **O que saiu de `folha_pj`:** as três ações que **desfazem** o que já foi pago — `corrigirPagamentoEfetivado` (F11), `estornarPagamentoEfetivado` (G1b) e `excluirFolhaProjetista` (F12). Pagar, editar pendente, cancelar pendente, gerar lote e anexar comprovante continuam em `folha_pj`.
- **Ninguém perdeu acesso:** a migration `20260912120000_perfis_folha_pj_corrigir` **deriva da tabela** (`SELECT ... FROM permissao_perfil WHERE acao = 'folha_pj'`), não de uma lista de chaves — quem paga hoje ganhou o par novo, inclusive em perfis customizados. Perfil que teve `folha_pj` revogado à mão não ganha nada. O mesmo vale para override individual (`permissao_usuario`), que leva junto `expiraEm` e `motivo`: sem isso, quem tinha liberação nominal com validade perderia corrigir/estornar sem ninguém decidir.
- **Por que migration, e não seed:** `seedPerfisAcesso` é create-only desde 2026-09-02 — par novo não chega sozinho a perfil existente, e nasceria negado para todo mundo em silêncio. Ver a memória `permissoes-matriz-legada-nao-autoriza`.
- **Tela:** `podeCorrigir` desce da página para os dois modos e para a aba Lotes. Sem o par, os botões "Corrigir pagamento", "Estornar" e a lixeira do lote **não aparecem** — e as actions recusam de qualquer jeito, que é o gate que vale.
...
**✅ G5 entregue 2026-09-12 (Opus 5) — recibo assinável + NF do PJ (D36):**

Decisões do dono nesta fase (2026-09-12): recibo **individual E mensal**; assinatura **dentro do sistema, logado**; **depois** do pagamento, sem travar nada. E, no meio da execução: **o recibo do PJ recebe a(s) NF(s)** daquele valor/mês.

- **Modelo:** `ReciboProjetista` (+ `ReciboProjetistaItem`). Guarda o **texto** exato assinado e o **SHA-256** dele — mesma prova do aceite de Termos (`modules/legal`). Só hash não permitiria reexibir o documento; só texto não detectaria adulteração. Tabela de itens (e não `reciboId` no pagamento) porque o mesmo pagamento entra no recibo individual **e** no consolidado do mês.
- **Regras puras** (`recibo/service.ts`, 13 testes): `textoRecibo` determinístico (nada de `new Date()` dentro — quem chama passa a data, senão o hash nunca bateria duas vezes), `totalRecibo` (valor nunca é digitado, sempre derivado), `competenciaRecibo` e `renderReciboHtml`, que **escapa** o texto gravado em vez de injetá-lo como HTML.
- **Gerar** é de quem paga (`folha_pj`): individual pelo ícone na linha paga, mensal pelo ícone no grupo do projetista (recorte por **data de pagamento** — o recibo é do dinheiro que saiu, não das entregas liberadas). Duplicata é recusada nos dois casos. O projetista é notificado (categoria `pagamento`).
- **Assinar** é do titular: gate `financeiro:extrato` (par que PJ/freelancer já têm) **mais** a checagem `recibo.projetistaId === user.id` — ninguém assina recibo alheio, nem quem manda no financeiro. Antes de assinar, o hash do texto gravado é reconferido; divergiu, não assina. `assinadoEm: null` na condição do update impede assinatura dupla por clique repetido.
- **PDF** (`/api/financeiro/recibos/[id]/pdf`): puppeteer-core + `setContent`, mesma mecânica da memória de cálculo. Renderiza o **texto gravado**, não um texto remontado na hora — o PDF tem de mostrar o que foi assinado, mesmo que a redação do gerador mude depois. Acesso: titular ou `folha_pj` (`financeiro:ver` não entra: recibo nominal não é relatório de caixa).
- **NF do PJ:** reusa `NotaFiscalPJ` e a rota `/api/rh/nf` que o PJ já usa — só entra o `reciboId`, com checagem de titular (sem ela, um PJ mandaria nota para o recibo de outro adivinhando o id). Várias NFs por recibo. A validação pelo RH segue igual, e o envio avulso continua existindo. `SetNull` no vínculo: apagar o recibo não pode apagar documento fiscal.
- **Tela do projetista** (`/financeiro`): lista de recibos com o texto expansível (é o que a assinatura cobre, então fica visível fora do PDF), botão de assinar, PDF, envio de NF e o código de verificação.
...
**✅ G10 entregue 2026-09-12 (Sonnet 5) — filtro "sem comprovante" (D33):**
- Checkbox "Só pagos sem comprovante" na barra de filtros da aba Pagamentos (`FolhaFiltros`) — a F8 já mostrava o aviso linha a linha, mas não dava para listar de uma vez os pagos sem conferir. Ligar o filtro força `status=pago` junto (pendente/cancelado não têm comprovante para conferir; deixar o status como estava faria a lista vir sempre vazia).
- `idsPagosSemComprovante`/`comFiltroSemComprovante` (`folha/queries.ts`) reusam a mesma mecânica de duas colunas soltas (`lancamentoId`/`pagamentoProjetistaId`) que `comLancamentos` já usa — sem FK entre as tabelas, então o cálculo é: achar os pagos do recorte, os lançamentos ligados a eles (por qualquer lado), e sobra quem tem 0 anexos ou nenhum lançamento.
- O filtro entra no `base` (não só no `where` final), então os 3 cards de KPI e a exportação (`dadosFolhaExport`, D10 — export = tela) também refletem o recorte estreito, igual a qualquer outro filtro da barra.
- `FiltrosFolha.semComprovante` novo, lido por `lerFiltrosFolha`, contado em `temFiltroAlemDoStatus` e no nome do arquivo exportado (`nomeArquivoExport`).

**Verificação (G10):** `tsc`, `eslint`, 150 testes (+3), `smoke:sync-pagamento` 19/19. Dois scripts de banco em dev, apagados ao final: um lendo os 9 pagos reais (todos hoje sem comprovante) e conferindo que `listarFolha`/`dadosFolhaExport` com o filtro batem exatamente com o conjunto esperado, os KPIs pendente/cancelado zeram, e a combinação contraditória `status=pendente&semComprovante=1` dá lista vazia sem quebrar; outro criando (e removendo depois) um `LancamentoAnexo` real num lançamento pago para confirmar que o ramo "tem anexo → some da lista" também funciona, já que nenhum dado real do dev exercitava esse lado.

**✅ G11 entregue 2026-09-12 (Sonnet 5) — lote visível do lado do pagamento (D34):**
- **Coluna implícita**: `CelulaPagamento` (aba Pagamentos, os dois modos) ganhou uma linha "lote abr/2026" com link pra `?aba=lotes&loteId=<id>`, quando o pagamento pertence a um — o vínculo só aparecia de dentro do lote (F10) até aqui. `INCLUDE_PAGAMENTO` passou a trazer `folha` (relação de verdade `folhaId`/`@relation`, diferente do lançamento — colunas soltas). `mostrarLote={false}` no painel do próprio lote (`folha-lotes-section.tsx`) evita repetir "lote abr/2026" em toda linha de uma tabela já rotulada assim no cabeçalho.
- **Filtro por lote** na barra de filtros da aba Pagamentos (`FiltrosFolha.folhaId`), com as mesmas opções (só lotes com pagamento) e entrando no `base` compartilhado — cards de KPI e export também respeitam.
- **Link → auto-abre o lote**: `FolhaLotesSection` recebe `loteAlvo` (de `?loteId=`) e repassa a cada `LinhaLote`; a linha que bate abre e rola até a vista sozinha (`autoAbrir`), com page-jump embutido em `listarFolhasProjetista` (conta quantos lotes vêm antes na ordenação e calcula a página, sem paginar tudo) — só ativo quando não veio `?page=` explícito, pra não puxar de volta quem já está paginando manualmente.
- **Export do conteúdo de um lote**: botões XLSX/CSV dentro do painel expandido, reusando a mesma rota da aba Pagamentos com `folhaId`+`status=todos`. Escondidos junto com "sem permissão" (piso de sócio da rota × `pagamentosDoLote` sem piso).
- **Achados do `advisor()`, corrigidos antes do commit:** (1) o nome do arquivo exportado por lote caía sempre em `Producao-todos-lote.xlsx` pra qualquer lote — exatamente a colisão que a própria função documenta evitar; corrigido com `loteRotulo` (mês-ano resolvido em `dadosFolhaExport`, não pelo cuid) passado a `nomeArquivoExport`. (2) os botões de export ficavam visíveis mesmo no ramo "sem permissão para ver os pagamentos deste lote" — corrigido escondendo-os junto.

**Verificação (G11):** `tsc`, `eslint`, 153 testes (+3), `smoke:sync-pagamento` 19/19. Dois scripts de banco em dev, apagados ao final: um checando, para o único lote real, a invariante "aparece na página que o page-jump escolhe" + `listarFolha(folhaId)` isolado + `dadosFolhaExport(folhaId)` == `listarPagamentosDoLote` + `loteRotulo` correto, e um `folhaId` inválido caindo no genérico sem quebrar; outro criando (e removendo depois) 13 lotes sintéticos sem pagamento pra forçar o page-jump atravessar de fato uma fronteira de página (pageSize=12) — o único jeito de testar a aritmética com o volume real do dev, que tem 1 lote só.

**✅ G12 entregue 2026-09-12 (Sonnet 5) — extrato do projetista com forma/anexo (D35):**
- `meuExtrato` (`financeiro/queries.ts`) passou a trazer, por pagamento, a **forma** de pagamento e os **anexos** do lançamento, com link de download — decisão do dono: **sem a conta bancária da empresa**. A query nunca seleciona `conta`; o campo simplesmente não existe no shape devolvido ao projetista.
- **Nota pro registro da decisão**: o dono aprovou omitir o CAMPO conta e aprovou o download em si. O ARQUIVO do comprovante (em geral o recibo do banco) pode conter a conta remetente por dentro do PDF/imagem — isso é do conteúdo do arquivo, fora do controle da tela, e não do que a decisão cobriu.
- **Rota de download reaproveitada**: `/api/financeiro/folha-projetistas/comprovante/[id]` (G6) ganhou uma segunda porta de entrada — além de `folha_pj` (staff), o **titular** do pagamento (resolvido pela cadeia `anexo → lançamento.pagamentoProjetistaId → pagamento.projetistaId`) também baixa, mesmo padrão "titular ou `folha_pj`" do PDF de recibo (G5). Falha de titularidade devolve 404 (não 403) — não revela a quem não tem acesso que o id existe.
- **Dois achados do `advisor()`:** (1) o rótulo da UI dizia "comprovante" pra qualquer `LancamentoAnexo` do lançamento — quem tem `financeiro:gerir` pode ter anexado algo ali que não é o comprovante do pagamento (mesma ressalva já documentada em `CelulaPagamento`, D26); agora é "Anexos deste pagamento", e o campo interno também virou `anexos` (era `comprovantes`). (2) **achado real, corrigido**: a resolução usava o mesmo OR de duas colunas soltas de `comLancamentos`, mas `PagamentoProjetista.lancamentoId` não tem `@unique` — uma anomalia de dado (dois pagamentos com o mesmo `lancamentoId`) faria a LISTAGEM do extrato de um projetista mostrar o anexo (nome + link) de outro, embora o DOWNLOAD em si já recusasse (a rota só confia no lado autoritativo `Lancamento.pagamentoProjetistaId`, `@unique`). Corrigido: a resolução agora prioriza o lado autoritativo e só confia na coluna solta se o lançamento não estiver "reivindicado" por outro pagamento.

**Verificação (G12):** `tsc`, `eslint`, 153 testes, `smoke:sync-pagamento` 19/19. Dois scripts de banco em dev, apagados ao final: um confirmando, pros 3 projetistas reais com pagamento, que os conjuntos de anexos vistos são disjuntos, a titularidade bate e nenhum campo de conta vaza no shape (vacuamente ok — dev não tem nenhum anexo real hoje); outro **forçando de propósito** a colisão (cria um anexo real no lançamento do projetista A, aponta temporariamente o `lancamentoId` solto de um pagamento do projetista B pro mesmo lançamento) — confirmou que, ANTES do fix, B via de fato o anexo de A na listagem, e que, DEPOIS do fix, B deixa de vê-lo enquanto A continua vendo o próprio normalmente. Tudo revertido no `finally`.

**✅ G13 entregue 2026-09-12 (Sonnet 5) — alerta semanal de pendente parado (D38):**
- `alertaPendenteParado` (novo, `jobs-handlers.ts`), agendado semanal segunda 08:15 (junto dos outros lembretes de segunda). Até aqui, "parado há N dias" era só um rótulo na linha da tela — só visto por quem abre a Produção.
- **Agregado, nunca um por pagamento** (decisão do dono: cuidado com ruído): uma notificação por gestor (`admin`/`supervisor`/`administrativo`, mesmo grupo de `alertaRateioAberto`) resumindo quantos pagamentos estão parados e a soma dos valores. Silêncio total quando não há nenhum (`return 0` antes de notificar qualquer coisa).
- `href` leva direto pra lista filtrada (`?status=pendente&sort=liberadoEm&dir=asc`) — os mais antigos no topo — não pra tela inteira agrupada.
- **Sem `categoria`/opt-out**, seguindo o precedente mais próximo (`alertaRateioAberto`, também sem categoria) — registrado aqui pra decisão poder ser revisitada; a memória `blueprint-decisoes-processos` registra "notif opt-out+alvo" como decisão de processo geral do dono, e este alerta ficou de fora dela por ora.
- **Achado real do `advisor()`, corrigido antes do commit**: a implementação original filtrava em JS chamando `diasPendenteParado` (a mesma função pura da UI) por pagamento. Essa função passa `liberadoEm` pela heurística de `paraData`/`inicioDoDia` — feita pra reconstruir campos de DATA gravados como meia-noite UTC (`@db.Date`), mas `liberadoEm` é um INSTANTE real (`@default(now())`); se por acaso caísse exatamente em meia-noite UTC, a heurística deslocaria o dia-calendário em fusos atrás de UTC (o mesmo tipo de bug já documentado no topo de `lib/data.ts` como incidente real de produção, e a mesma classe do achado da G8 com `pagoEm`). Um alerta semanal transformaria essa instabilidade rara em notificação que aparece/some entre semanas. Corrigido: o filtro virou uma comparação de INSTANTE direto no `where` do Prisma (`liberadoEm < inicioDoDiaLocal(agora) - (N-1) dias`), sem nunca reler os componentes de `liberadoEm` em JS — `diasPendenteParado` continua intocada (a UI depende dela). Segundo achado, mesmo commit: o `href` caía na tela inteira em vez da lista já filtrada.

**Verificação (G13):** `tsc`, `eslint`, suíte completa do repo (278 arquivos, 3021 testes), `smoke:sync-pagamento` 19/19. Script de banco em dev, apagado ao final: criou 3 pagamentos sintéticos reais (valor R$0, não distorce nenhum agregado financeiro) com `liberadoEm` cravado em N-1/N/N+1 dias — confirmou que o `where` do banco bate exatamente com o esperado nos 3 casos; chamou `alertaPendenteParado()` de verdade (manda notificação real aos gestores do dev) e confirmou que cada um dos 3 gestores recebeu **exatamente 1** notificação nova, mesmo com 6 parados no total (prova a agregação, não fan-out por pagamento). Notificações de teste e pagamentos sintéticos removidos no `finally`, limpeza confirmada por contagem zero depois.

**✅ G14 entregue 2026-09-12 (Sonnet 5) — testes de integração das guardas F11/F12 (D39):**
- Novo `folha/actions.test.ts` (14 testes), mockando sessão/permissão/`prisma`/auditoria/notificação — mesmo padrão de `rh/pessoas/queries.test.ts` (`vi.mock("@/lib/prisma", ...)` com métodos individuais). Cobre `corrigirPagamentoEfetivado` (F11) e `estornarPagamentoEfetivado` (F12): permissão `folha_pj_corrigir` explicitamente asserida, guarda de leitura (status/conciliado/parcial) bloqueando ANTES de qualquer escrita, guarda repetida NA ESCRITA (`transacao: { is: null }`/`{ is: { id } }` no `where` do `updateMany`), caminho feliz conciliado-batendo (G1a — `pagoEm`/`dataConfirmacao` vêm da transação, não do formulário, com data deliberadamente diferente pra discriminar), notificação G8 só quando valor/data mudam, e os dois `count === 0` (concorrência: "mudou enquanto a tela estava aberta").
- Escopo deliberadamente restrito a estas duas actions (as citadas na razão do D39) — não é uma suíte de integração do módulo inteiro; as outras actions de `folha/actions.ts` continuam cobertas só pelas regras puras (`service.test.ts`) e pela verificação manual em banco de cada fase.
- **Três achados do `advisor()`, todos sobre o mesmo risco (teste que sobrevive à remoção da guarda que deveria travar), corrigidos antes do commit:** a permissão não era asserida (só `mockResolvedValue(true)` global — trocar `folha_pj_corrigir` por `folha_pj` na action passaria batido); a guarda de escrita não era checada (só o `count` do retorno mockado — apagar a cláusula `transacao: {is: null}` do `where` real passaria batido); faltava o caminho feliz conciliado-BATENDO (só o divergente/recusado estava coberto — a feature em si da G1a não tinha teste nenhum).

**Verificação (G14):** `tsc`, `eslint`, suíte completa (279 arquivos, 3035 testes), `smoke:sync-pagamento` 19/19. Prova por mutação (não script de banco — este ganho não muda runtime, só adiciona testes): removeu de propósito as duas cláusulas de guarda-na-escrita da action real, rodou a suíte — 3 dos 14 testes falharam exatamente como esperado — e restaurou o arquivo original (`git diff` limpo depois).

**✅ G15 entregue 2026-09-12 (Sonnet 5) — manual atualizado (D32, por último):**
- `docs/manual/financeiro/producao.md` reescrito para refletir tudo de F8 a G14: comprovantes (anexar no ato de pagar/em lote/retroativo, filtro "sem comprovante"), recibos (geração, assinatura do projetista, PDF, NF), corrigir/estornar pagamento já pago (regra do conciliado batendo, notificação G8), lote (mover pendente, ações dentro do lote expandido, excluir lote, export do lote, link "lote X/Y" da aba Pagamentos), alerta semanal de pendente parado, e a tabela de permissões separando `folha_pj` de `folha_pj_corrigir`/`conciliar`. FAQ ganhou 4 perguntas novas.
- `docs/manual/financeiro/visao-geral.md` (seção "Meu extrato"): uma frase sobre forma/anexos aparecerem no extrato do projetista, sem a conta bancária da empresa (D35/G12).
- `docs/manual/search-index.json` atualizado junto (descricao/resumo/palavrasChave da entrada de `financeiro/producao.md`) — sem isso a página ficaria invisível na busca do `/ajuda`, que é visível a todos os papéis, cliente incluído.

**Verificação (G15):** sem `tsc`/`eslint` (nenhum código mudou) — a checagem equivalente foi um script temporário (`_tmp-verificar-manual*.ts`, apagados ao final) usando `lib/manual.ts` de verdade: `lerPaginaManual` carrega a página, `lerManifesto` traz a entrada atualizada, e `buscarManual` encontra `financeiro/producao` tanto por "comprovante" quanto por "recibo". Suíte completa do repo rodada de novo por precaução (279 arquivos, 3035 testes, inalterada) e `smoke:sync-pagamento` 19/19.
