  ...
  const automacoes: { fila: string; cron: string; handler: () => Promise<unknown> }[] = [
    ...
    },
    {
      ...
      handler: async () => {
        ...
        console.log(`[rh] propostas vencidas=${r.propostas} férias iniciando=${r.ferias} contratos de equipe vencendo=${r.contratosEquipe} vínculos encerrados=${r.vinculosEncerrados} acessos encerrados=${r.acessosEncerrados}`);
      },
    },
    {
      fila: "resumo-semanal",
