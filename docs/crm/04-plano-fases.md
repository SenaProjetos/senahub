...
| F1.7 | Chaves em `ConfigSistema` (padrão de ~8 módulos): `comercial.descontoMaxSemJustificativa=10` (Q6), `comercial.diasSemContato`, `comercial.diasAvisoValidadeProposta`, `comercial.diasClienteInativo` | F1.6 | `grep` não acha nenhum desses números literal em `src/modules/comercial/` ✅. **Emenda:** a parte "seed roda 2×" não se aplica — o padrão real dos ~8 módulos que usam `ConfigSistema` é **não semear** (default no código; a chave só nasce quando alguém edita na tela). Semear duplicaria a fonte do default. Coberto por teste puro do parsing em vez de smoke | — | | H | **S** | smoke |
...
| F1.15 | **Produção:** validar backup (`pg_restore --list` enumera `lead`/`cliente`/`proposta`/`anexo_lead`/`projeto`; espelho de `STORAGE_BASE_PATH` em dia) e **fundir os grupos duplicados**, conferindo os projetos de cada grupo **antes**. Conferido contra produção em 2026-08-19: são **4 grupos-registro** — MADANO ×2, **Záphis ×4** (o §4 dizia 3; `Zaphis Inc LTDA`, dono dos 2 projetos e do CNPJ, não casava na normalização de nome e só apareceu na conferência manual — mesma empresa confirmada pelo usuário), Nominal ×2 | F1.14 | **5 fusões** (não 3 — Záphis absorve 3): `cliente` mantém **46 linhas** (fusão arquiva, não apaga) e cai de 46 para **41 não-fundidos**; `projeto` continua **32** (não 31 — o número do §7 é de 2026-08-14 e já havia envelhecido); retrato `projeto→clienteId` antes/depois prova que **só o projeto previsto trocou de cliente**; **5** entradas de fusão no `AuditLog` | 03-mig §4 | ⚠️⚠️ **produção** | **O** | — | smoke |
| F1.16 | Índice único parcial `CREATE UNIQUE INDEX cliente_documento_unico ON cliente (documento) WHERE documento IS NOT NULL` + mensagem de negócio no cadastro. **Precedido de `scripts/normalizar-documento-f116.ts`**: `documento = ''` NÃO é NULL e é pego pelo predicado — havia **2 clientes PF assim em produção**, que colidiriam entre si e fariam o `CREATE INDEX` abortar; o mesmo script tira a pontuação de **4** CNPJs, sem o que o índice deixaria o mesmo CNPJ passar escrito de dois jeitos | **F1.15** (a fusão Nominal *preenche* um documento que faltava — criar antes arrisca falhar ou travar a fusão) | migration aplica sem erro; tentar salvar 2º cliente com o mesmo CPF/CNPJ é recusado com mensagem de negócio que **diz em qual cliente o documento já está** | ADR-03 | ⚠️ produção | S | **M** | browser |
...
| **F1.19c** | **Lacuna descoberta na F1.21 (2026-08-19):** `Disciplina.disciplinaId` FK → `DisciplinaCatalogo` + `disciplinaTextoLegado`. F1.19 fez isso em `PropostaItem` e F1.20 em `ItemTabelaPreco`, mas **ninguém fez em `Disciplina`** — que é justamente onde vivem as grafias livres, e a tabela que carrega `valor` (pagamento ao projetista), `RevisaoDisciplina`, uploads e apontamentos. Sem esta FK o aceite da F1.21 é impossível de cumprir. Exige migration + ajustar o que lê `Disciplina.nome` no módulo **projetos** (fora do CRM) | F1.20 | `prisma validate`; `/projetos/[id]` continua listando as disciplinas; nenhuma `Disciplina` perde `nome` (o texto vai para `disciplinaTextoLegado`) | Q3 | ⚠️⚠️ respinga no módulo projetos, fora do CRM | **O** | **M** | smoke |
| F1.21 | **Consolidação das grafias de `Disciplina` em produção.** Números conferidos contra produção em 2026-08-19: são **18 grafias distintas** (não 24 — aquele número somava as três tabelas), das quais **12 batem exatas** com o catálogo; `Ar condicionado (ARC)`+`Exaustão (EXT)`→`Climatização (AVAC)`, `Gases`→`Gás`; as **3 strings compostas** (`Lógica/cftv`, `Lógica e Cftv`, `Dados/Voz, Automação e CFTV`) → `Cabeamento` + `CFTV` **à mão, com o responsável do projeto**. `PropostaItem` e `ItemTabelaPreco` **já estão 100% preenchidos** (0 nulos, medido) — o que resta é só `Disciplina` | **F1.19c** | zero `Disciplina`/`PropostaItem`/`ItemTabelaPreco` sem `disciplinaId`; soma de `Disciplina.valor` por projeto **inalterada** antes/depois; nenhuma `RevisaoDisciplina` perdida (contagem igual) | Q3, 03-mig §5 | ⚠️⚠️ `Disciplina` carrega **pagamento ao projetista** | **O** | — | smoke |
...
| F7.9 | **Página de ajuda do Comercial** — reescrever `docs/manual/clientes-comercial/comercial.md` + `guia-iniciante.md` (já existem, mas descrevem o módulo PRÉ-reforma: "Oportunidades", funil único, sem Prospecção/Negociação) cobrindo o fluxo inteiro pós-reforma passo a passo: Prospecção → sinal de reativação (F3.8) → Negociação → Proposta → Projeto; e as ferramentas — registro manual de interação (F3.4), templates de nota (F3.5), timeline unificada (F3.6), Empresa 360 (F3.7), próxima ação/follow-up. Sem tela nova nem rota nova — o `/ajuda` (`lib/manual.ts`) já renderiza `docs/manual/**`, visível a todos os papéis | tudo | usuário sem contexto lê o manual e sabe **onde** registrar um contato, **quando** uma prospecção vira negociação, e **o que** o sinal de reativação está oferecendo; `grep -i "oportunidade"` no arquivo reescrito não acha nada (termo morto pela F1.x) | (pedido do dono, 2026-08-21) | | S | — | browser |
| F7.10 | Fecho: lint + test + build; marcar o roadmap A–F como **superseded** apontando para este arquivo; entrada final no `06-progresso.md` | F7.9 | cabeçalho do `2026-07-24-crm-comercial-roadmap.md` diz "superseded por `docs/crm/04-plano-fases.md`" | — | | H | — | — |
