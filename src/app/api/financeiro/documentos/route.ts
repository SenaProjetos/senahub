...
/** Recebe o arquivo do documento financeiro (NF/contrato) e devolve a metadata. */
export async function POST(req: Request) {
  ...
  const rel = `financeiro/docs/${randomBytes(12).toString("hex")}${ext}`;
  await salvarArquivo(rel, Buffer.from(await file.arrayBuffer()));
  return NextResponse.json({ caminho: rel, nomeArquivo: nome, mime: file.type || "application/octet-stream" });
}
