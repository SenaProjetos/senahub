...
| # | Divergência | Onde | Situação |
| --- | --- | --- | --- |
| F3-1 | Ajuste do **próprio** ponto é aplicado **sem ciência** e sem fila de aprovação — só com justificativa. Ajuste feito por **gestor** também aplica na hora, mas cria `pendente_ciencia` e notifica o colaborador para confirmar ou contestar. O rascunho do guia dizia "precisa de aprovação"; estava errado. | `modules/ponto/actions.ts:225-265` | **Corrigida no próprio guia.** Nenhuma mudança de código — a assimetria é deliberada e boa (ninguém mexe no seu ponto sem você saber). Registrada porque é fácil de supor errado. |
| F3-2 | O cálculo de férias é declaradamente **simplificado**: 12 meses → 30 dias, janela concessiva de 12 meses, sem faltas que reduzem o direito, sem fracionamento e sem abono. | `lib/aquisitivo.ts` | **Aberta, por design.** O guia e o stub avisam explicitamente que não é orientação trabalhista. Se o escritório precisar da regra completa, é feature, não correção. |
| F3-3 | As faixas de INSS/IRRF **não vêm embutidas**: com `EncargoFaixa` vazio o motor devolve zero, em silêncio. Um encargo zerado parece cálculo quebrado e é configuração faltando. | `lib/encargos.ts`, Configurações → Encargos | **Aberta, baixa prioridade.** Documentado como dúvida no guia; a tela poderia avisar quando não há faixas cadastradas. |

**Nota de gate:** `/rh` **não** recebeu `mostrarGuia`. O `requireRole` da própria página já admite só
