...
export function FunilComercialBoard({
  ...
  /**
   * Ponto único do arrasto e do menu de contexto: as duas superfícies entram por aqui, então a
   * regra (`decidirSoltura`), os diálogos e a confirmação de reativação valem igual nas duas.
   */
  async function pedirMover(card: CardFunil, destino: ColunaFunil) {
        ...
        <div className="flex flex-col gap-3 pb-2 sm:flex-row sm:overflow-x-auto">
          {exibidas.map((c, i) => (
            <Coluna
              ...
              agora={agora}
              alvoId={alvoId}
              onMover={(card, destino) => void pedirMover(card, destino)}
            />
          ))}
        </div>
        <DragOverlay>
